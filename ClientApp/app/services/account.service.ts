...
export class AccountService {
    ...
    getInfo(uid: any): Observable<any> {
        return this.http.get(this.baseUrl + '/getInfo/' + uid, { headers: AuthService.credentialHeader() })
            .map(this.extractData)
            .catch(err => this.toaster.popAsync('error', "Get Info", "Failed!"));
    }

    editInfo(user: any): Observable<boolean> {
        return this.http.put(this.baseUrl + '/edit/' + user.id, JSON.stringify(user), { headers: AuthService.credentialHeader() })
            .map(response => {
                if (response.ok) {
    ...
    getRole(roleId: number): Observable<any> {
        return this.http.get(this.baseUrl + "/getRole/" + roleId, { headers: AuthService.credentialHeader() })
            .map(this.extractData)
